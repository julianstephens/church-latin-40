...
export function sanitizeOption(option: string): string {
  ...
  // For options, we mainly need to escape HTML special characters that pose XSS risks
  ...
  return option.trim().replace(/[&<>]/g, (char) => map[char]);
}
...
export function normalizeAnswerForComparison(answer: string): string {
  ...
  return answer.toLowerCase().trim().replace(/\s+/g, " "); // Normalize multiple spaces to single space
}
