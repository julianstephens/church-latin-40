...
class VocabularyService {
    ...
    async getVocabByLesson(lessonId: number): Promise<VocabWord[]> {
        try {
            const records = await pb.collection("church_latin_vocabulary").getFullList({
    ...
    async getVocabWord(wordId: string): Promise<VocabWord | null> {
        try {
            const record = await pb.collection("church_latin_vocabulary").getOne(wordId);
     ...
     */
    async selectRandomWords(lessonId: number, count: number): Promise<VocabWord[]> {
    ...
    generateMultipleChoiceQuestion(
        ...
        if (!word) {
            throw new Error("Cannot generate multiple-choice question without a word");
        }
     ...
     */
    async generateQuestion(options: GenerateQuestionOptions): Promise<GeneratedVocabQuestion> {
     ...
     */
    evaluateTranslationAnswer(correctAnswer: string, userAnswer: string): boolean {
