...
export class ReviewService {
   ...
   * @param questionId Stable question ID
   */
  async handleQuizMiss(lessonId: number, questionId: string): Promise<void> {
    try {
      const authStore = pb.authStore;
      if (!authStore.isValid || !authStore.model) {
        throw new Error("User not authenticated");
      ...
      const existing = await pb
        .collection("church_latin_review_items")
        .getList(1, 1, {
          filter: `userId = "${userId}" && lessonId = "${pbLessonId}" && questionId = "${questionId}"`,
        });
      ...
      if (existing.items.length > 0) {
        ...
        await pb.collection("church_latin_review_items").create({
          userId,
          lessonId: pbLessonId,
          questionId,
