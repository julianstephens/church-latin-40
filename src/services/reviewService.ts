...
export class ReviewService {
     ...
     * @param questionId Stable question ID
     */
    async handleQuizMiss(lessonId: number, questionId: string, vocabWordId?: string): Promise<void> {
        try {
            const authStore = pb.authStore;
            if (!authStore.isValid || !authStore.model) {
                throw new Error("User not authenticated");
            ...
            if (existing.items.length > 0) {
                ...
                await pb.collection("church_latin_review_items").create(reviewItemData as never);
                console.debug(
