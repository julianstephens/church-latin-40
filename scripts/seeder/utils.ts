...
export async function getPocketBase(): Promise<PocketBase> {
  ...
  if (!adminEmail || !adminPassword) {
    logWarn(
      "⚠️  WARNING: Admin credentials not provided. Set POCKETBASE_ADMIN_EMAIL and POCKETBASE_ADMIN_PASSWORD environment variables.",
    );
  ...
  try {
    ...
    logError(
      `❌ Failed to authenticate with PocketBase admin account: ${message}`,
    );
...
export async function clearCollection(
  ...
  try {
    const records = await (await getPocketBase())
