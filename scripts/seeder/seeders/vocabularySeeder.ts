...
import type { ISeeder, SeedOptions, SeedResult, VocabularyData } from "../types";
import {
    clearCollection,
    createSeedResult,
    getPocketBase,
            ...
            for (const vocabData of vocabulary) {
                ...
                try {
                    ...
                    try {
                        ...
                        logVerbose(`Could not find lesson with number: ${lessonNumber}`, options);
                    }
                    ...
                    const validPoS = ["noun", "verb", "adjective", "adverb", "preposition", "conjunction", "pronoun", "article"];
                    const validFrequency = ["high", "medium", "low", "unknown"];

                    // Use "unknown" as default if frequency is missing or invalid
                    const frequency = vocabData.frequency && validFrequency.includes(vocabData.frequency.toLowerCase())
                    ...
                    if (vocabData.partOfSpeech && validPoS.includes(vocabData.partOfSpeech.toLowerCase())) {
                    ...
                    try {
                        ...
                        logVerbose(`Record ${lookupId} exists with ID ${existingId}`, options);
                    } catch (checkError) {
                        exists = false;
                    ...
                    if (exists) {
                        ...
                        updated++;
                        if (options.dryRun) {
                            logVerbose(`[DRY RUN] Would update word ${vocabData.word}`, options);
                        } else {
                        ...
                        added++;
                        if (options.dryRun) {
                            logVerbose(`[DRY RUN] Would create word ${vocabData.word}`, options);
                        } else {
