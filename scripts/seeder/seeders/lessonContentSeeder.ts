...
import type { ISeeder, LessonContentData, SeedOptions, SeedResult } from "../types";
import {
    clearCollection,
    createSeedResult,
    getPocketBase,
...
export class LessonContentSeeder implements ISeeder {
    ...
    async seed(options: SeedOptions): Promise<SeedResult> {
        ...
        try {
            ...
            const contentItems = readJsonData<LessonContentData>("lesson-content.json");
            ...
            for (const contentData of contentItems) {
                ...
                try {
                    ...
                    if (exists) {
                        ...
                        updated++;
                        if (options.dryRun) {
                            logVerbose(`[DRY RUN] Would update content for lesson ${contentData.lessonId}`, options);
                        } else {
                            logVerbose(`Updated content for lesson ${contentData.lessonId}`, options);
                        }
                    } else {
                        if (!options.dryRun) {
                            (await getPocketBase())
                        ...
                        if (options.dryRun) {
                            ...
                            logVerbose(`Created content for lesson ${contentData.lessonId}`, options);
                        }
                    }
                } catch (error) {
